...
/*
并发编程
goroutine是Go并行设计的核心. goroutine是协程, 比线程更小, 十几个goroutine在底层就是五六个线程,
Go语言内部实现了这些goroutine之间的内存共享. 执行goroutine只需极少的栈内存(大概是4~5KB),
正因为如此, 可同时运行成千上万个并发任务. goroutine比thread更易用,更高效,更轻便.
*/
/*
...
func recvView(ch chan []string) {
	for i := 0; i < 3; i++ {
		val := <-ch
		fmt.Println(i, "recv =", val, "len =", len(val))
	}
}
func testChnal() {
	...
	go recvView(ch)
	str := "123"
	ch <- strings.Split(str, "1")
	ch <- strings.Split(str, "2")
	ch <- strings.Split(str, "3")
}
